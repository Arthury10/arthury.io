    ...
    <section id='contact'>
      ...
      <div className="container contact__container">
            ...
            {contacts.map((item) => (
              <article className="contact__option"> 
                ...
                {item.title === 'WhatsApp' && (
                  ...
                  <a href={item.href} target="_blank" rel="noreferrer">{item.btn}</a>
              </article>
            ))}
        </div>
        ...
        <form 
          ...
          {item.type === "textarea" && (
            <textarea name={item.name} placeholder={item.placeholder} required />
          )}
          </>
          ))}
          <button type="submit" className='btn btn-primary'>{btn.value}</button>
        </form>
      </div>
    </section>
  )
