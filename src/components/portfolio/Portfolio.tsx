...
const Portfolio = () => {
  ...
  useEffect(() => {
    async function getRepositories() {
      const response = await fetch('https://api.github.com/users/arthury10/repos')
      ...
      {repositories.map((repository: PortfolioProps) => (
        <SwiperSlide key={repository.id} className='portfolio__item'>
            ...
            <div className="portfolio__item-cta">
              <a href={repository.html_url} className='btn' target='_blank' rel="noreferrer">Github</a>
              {!repository.has_pages ? null : (
              <a href={`https://arthury10.github.io/${repository.name}/`} className='btn btn-primary' target='_blank' rel="noreferrer">Page</a>
              )}
            </div>
            <div>
            </div>
